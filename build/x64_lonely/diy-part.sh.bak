...
uci set network.lan.gateway='10.10.10.250'                    # 旁路由设置 IPv4 网关（去掉uci前面的#生效）
#uci set network.lan.broadcast='10.10.10.250'                 # 旁路由设置 IPv4 广播（去掉uci前面的#生效）
uci set network.lan.dns='61.139.2.69 218.6.200.139'           # 旁路由设置 DNS(多个DNS要用空格分开)（去掉uci前面的#生效）
...
uci set system.@system[0].hostname='OpenWrt-Lonely'           # 修改主机名称为OpenWrt-123
...
sed -i "s/OpenWrt /X86旁路由 compiled in by Lonely-Esc $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" "$ZZZ_PATH"
